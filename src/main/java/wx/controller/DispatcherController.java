...
//                                //资源流动结束，返回到这里
//                                deferredResult.setResult(new LFThrowable.Builder().code(0).desc("Success").build().toJsonString());
//                            }
//                        },
//                        (throwable) -> {
//
//                            if (!deferredResult.hasResult()) {
//                                deferredResult.setResult(new LFThrowable.Builder().code(ErrorConfig.CODE_1005).desc("Error,输入的路径有误或发生内部错误!").build().toJsonString());
//                            }
//
//                        });
            ...
            deferredResult.setResult(new LFThrowable.Builder().desc("No Resource Query!").build().toJsonString());
