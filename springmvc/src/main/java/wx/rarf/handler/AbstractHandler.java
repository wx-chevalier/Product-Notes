...
import wx.controller.DispatcherController;
    ...
    public Observable<ResourceBag> handle(ResourceBag resourceBag) {
        ...
        return Observable.create(
                (subscriber) -> {
                    ...
                    //判断route方法中执行结果
                    //首先判斷是否為ThrowAble類型
                    if (resultFromInnerRoute instanceof RARFThrowable) {

                        //设置本层的错误提示，考慮到結果只會被處理一次，因此會在这里先设置返回结果
                        resourceBag.setResultAndResource(((RARFThrowable) resultFromInnerRoute).getJSONObject(), null);
...
//                        this.getDispatcherController().getRelationProxy().combinedReduce(resourceBag).subscribe();
                        ...
                        //返回上一层的错误调用
                        subscriber.onError((RARFThrowable) resultFromInnerRoute);
                            ...
                            dispatcherController.forward(resourceBag)
                                    .subscribe(
                                            ...
                                            throwable -> {
                                                ...
                                                if (!(throwable instanceof RARFThrowable)) {
                                                    //打印错误
                                                ...
                                                resourceBag.setResultAndResource(
                                                        (new RARFThrowable.Builder().code(ErrorConfig.CODE_INTERNEL_EXCEPTION).desc(throwable.getMessage()).build()).getJSONObject()
                                                        , null);
...
//                            this.getDispatcherController().getRelationProxy().combinedReduce(resourceBag).subscribe();
    ...
    /**
     * @param resourceBag
     * @return 如果正常的话，返回true,如果有异常，返回RARFThrowable
     * @required
     ...
     * @param resourceBag
     * @return
     * @throws RARFThrowable 出现异常则抛出
     * @function 建议性，获取所有的ID
     */
    protected abstract JSONArray getIds(ResourceBag resourceBag, List<T> ownedResourceEntityList) throws RARFThrowable;

    /**
     * @return
     * @throws RARFThrowable 当获取实体为空时则报错
     * @function 建议性，根据ID获取所有的资源
     * @segment 遍历jsonArray获取所有尚未拥有的资源编号 -> 根据编号获取资源列表
     */
    protected abstract List<T> getResourceEntity(JSONArray jsonArray, ResourceBag resourceBag, List<T> ownedResourceEntityList) throws RARFThrowable;
     ...
     * @function 处理单个的创建请求
     */
    protected abstract void postSingle(ResourceBag resourceBag) throws RARFThrowable;

    /**
     * @param resourceBag
     * @throws RARFThrowable
     * @function 处理多个创建的请求
     */
    protected abstract void postMultiple(ResourceBag resourceBag) throws RARFThrowable;
     ...
     */
    protected abstract void setResult(ResourceBag resourceBag) throws RARFThrowable;
